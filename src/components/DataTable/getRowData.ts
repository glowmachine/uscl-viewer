...
export default function getRowData(data: Legislator[]): Row[] {
    const selectedData = data.map(member => {
        ...
        return {
            ...
            votesmart: member.id.votesmart ? String(member.id.votesmart) : ' ',
            // fec: string[] || ' ',
            cspan: member.id.cspan ? String(member.id.cspan) : ' ',
            wikipedia: member.id.wikipedia || ' ',
            house_history: member.id.house_history ? String(member.id.house_history) : ' ',
            ballotpedia: member.id.ballotpedia || ' ',
            maplight: member.id.maplight ? String(member.id.maplight) : ' ',
            icpsr: member.id.icpsr ? String(member.id.icpsr) : ' ',
            wikidata: member.id.wikidata || ' ',
            google_entity_id: member.id.google_entity_id || ' ',
            pictorial: member.id.pictorial ? String(member.id.pictorial) : ' ',
            ...
            class: String(currentTerm.class) || ' ',
            url: currentTerm.url || ' ',
